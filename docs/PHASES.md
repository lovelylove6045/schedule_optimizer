...
- [x] Screen 3 — [`step-academic-goals.tsx`](../frontend/src/components/wizard/step-academic-goals.tsx): add a second major/minor/emphasis. `academic_program_relationships` has zero rows loaded, so the picker shows every other program instead of filtering by relationship (documented simplification, same "real data if present, sensible default otherwise" pattern as earlier phases); interest tags are omitted entirely since `course_tags` is likewise empty (same known gap as Phase 3's unimplemented `MAX_INTEREST_ALIGNMENT`).
...
- [x] Screen 5 — [`step-objective-selection.tsx`](../frontend/src/components/wizard/step-objective-selection.tsx): the 5 supported `OptimizationObjectiveType`s as a ranked list with up/down buttons (the doc's own "simple ranked list" fallback instead of drag-and-drop, avoiding a new dependency) → submitted as `scenario_objectives` with `display_order`.
...
- [x] **Add/remove courses on the Plan Board (Screen 6)**, alongside swap — the same `plan_swap_validation` checks (offered-in-term, credit cap, prerequisites) were generalized (`validate_swap`/`validate_add` sharing one `_validate_course_for_slot` helper) so an add is held to the same standard as a swap:
  ...
  - [x] [`DELETE /plans/{id}/courses/{plan_course_id}`](../backend/app/routers/plans.py) ([`plan_swap_service.remove_plan_course`](../backend/app/services/plan_swap_service.py)) — deletes a course (and its `requirement_allocations`) entirely, via [`remove-course-button.tsx`](../frontend/src/components/plans/remove-course-button.tsx)'s trash icon on each course tile. No validation needed: removing a course can only make coverage worse, never invalid.
